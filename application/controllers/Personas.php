		...
		if ($this->session->userdata('rol') != 1) {
			...
			if ( $this->form_validation->run() == FALSE ) {
	          ...
	          if ( $this->Persona_model->tiene_archivo( $id,'pdf_dni') ) {
	          	...
	          	$this->Fileutil->actualizar_archivo( $archivo_id, $_FILES['pdf_dni'] );
	          } else {
	          	$this->Fileutil->subir_archivo( $_FILES['pdf_dni'], 'personas', 'personas', $id, 'pdf_dni' );
	          }
	        }
	        if ( isset( $_FILES['pdf_cuil'] ) ) {
	          if ( $this->Persona_model->tiene_archivo($id,'pdf_cuil') ) {
	          	...
	          	$this->Fileutil->actualizar_archivo( $archivo_id, $_FILES['pdf_cuil'] );
	          } else {
	          	$this->Fileutil->subir_archivo( $_FILES['pdf_cuil'], 'personas', 'personas', $id, 'pdf_cuil' );
	          }
	        }
	        if ( isset( $_FILES['pdf_alta_temprana'] ) ) {
	          if ( $this->Persona_model->tiene_archivo($id,'pdf_alta_temprana') ) {
	          	...
	          	$this->Fileutil->actualizar_archivo( $archivo_id, $_FILES['pdf_alta_temprana'] );
	          } else {
	          	$this->Fileutil->subir_archivo( $_FILES['pdf_alta_temprana'], 'personas', 'personas', $id, 'pdf_alta_temprana' );
	          }
	        }
